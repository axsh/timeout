...
build_root_module() {
    ...
    step "Root module: github.com/axsh/timeout"
    cd "$PROJECT_ROOT"

    info "Running Go unit tests with race detector..."
    UNIT_PKGS=$(go list ./... 2>/dev/null | grep -v '/tests$' | grep -v '/tests/' || true)
    if [[ -z "$UNIT_PKGS" ]]; then
        warn "No Go unit test packages found at repository root."
    elif echo "$UNIT_PKGS" | xargs go test -race -v -count=1; then
...
build_features() {
    ...
    for feature_dir in features/*/; do
        ...
        info "Running Go unit tests for $feature_name..."
    ...
    if [[ "$found_any" == "false" ]]; then
        info "No Go projects found under features/*/ (optional)."
    fi
}
...
build_go() {
    ...
    build_root_module || return 1
    build_features || return 1
}
